package io.micronaut.starter.analytics.services;

import io.micronaut.core.annotation.Nullable;
import io.micronaut.starter.analytics.configuration.StarterAnalyticsConfiguration;
import io.micronaut.starter.analytics.repositories.DailyRepository;
import io.micronaut.starter.analytics.services.charts.DailyBarChart;
import io.micronaut.starter.analytics.services.charts.Day;
...
public class DailyService {
    ...
    public DailyService(StarterAnalyticsConfiguration configuration,
    ...
    public List<DailyDTO> dailyStats(
        ...
        LocalDate from = fromQuery != null ? fromQuery : to.minusDays(days != null ? days : configuration.getDays() - 1);
        if (from.isAfter(to)) {
            LOG.error("Requested from date is after to date: {} > {}", from, to);
            throw new IllegalArgumentException("from date is after to date");
        }
        if (DAYS.between(from, to) > configuration.getMaxDays()) {
            LOG.error("Requested too many days: {}", DAYS.between(from, to));
            throw new IllegalArgumentException("too many days (max " + configuration.getMaxDays() + ")");
        }
